/**
 * Adornment SVG definitions for MagicalText.
 *
 * Adding a new adornment:
 * 1. Add its key to `AdornmentKind` in types.ts.
 * 2. Add an entry to `ADORNMENTS` here.
 * 3. Set `animationType`:
 *    - 'sparkle': rotate+scale animation, fits small geometric shapes.
 *    - 'scale': scale-only animation, fits detailed/illustrative shapes.
 * 4. Set `colorTracked: true` if you want the sparkle fill to mirror the
 ...
 *
 * SVG authoring notes:
 * - Use `aria-hidden="true"` - adornments are decorative.
 * - For color-tracked adornments: put `data-sparkle-path` on the path whose
 *   `fill` will be updated each animation frame. Initial fill value doesn't
 *   matter (overwritten immediately by JS).
 * - For static adornments: hardcode fills directly in the SVG markup.
 */

import type { AdornmentKind, AnimationType } from "./types";

export type AdornmentConfig = {
	/** Inline SVG string rendered into each sparkle wrapper. */
	svg: string;
	/** Default animation style for this adornment. */
	animationType: AnimationType;
	/** Whether the JS rAF loop updates the fill to match gradient position. */
	colorTracked: boolean;
};

// Star-cross
const STAR_SVG = `<svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path data-sparkle-path d="M 9.5, 6.5 8, 0 6.5, 6.5 0, 8 6.5, 9.5 8, 16 9.5, 9.5 16, 8 Z" fill="currentColor"/></svg>`;
